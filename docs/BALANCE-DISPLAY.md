...
顶栏现有布局：`[标题][工具按钮×5（…菜单）][系统按钮]`。
余额文本放在**菜单按钮右侧、系统按钮左侧**：

```
… [刷新][浏览器][日志][重启服务][菜单]  ¥ 23.50   ─ □ ✕
```

| 项 | 规范 |
|---|---|
| 文本 | `¥ 23.50`，12px，弱文字色（DynamicResource，深色 `#6E7681`），Padding 左右 8 |
| 对齐 | 固定高容器 + `VerticalAlignment="Center"`（R5：StackPanel 内 Stretch 贴顶陷阱）；单行文本不设 LineHeight |
...
| 与 R6/R7 的关系 | 余额是**显示区非工具按钮**，不进 5 按钮清单；错误态（服务断连）下余额照常显示（独立于服务，R7 精神一致） |
...
8. **回归**：主题三模式（余额文字随 DynamicResource 变色）、顶栏 5 按钮、R5 对齐不受影响
