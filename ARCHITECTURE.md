...
> 更新时间:2026-08-21 · 版本 v1.5.0 · 本次更新:菜单/弹窗动画体系（PopupAnimator：菜单默认档 / 状态卡轻量档 / 启动页入场编排）、像素点阵进度条（MainWindow.Progress）、Kimi 双窗口配额 + 刷新失败闭环（RefreshFailed）、整体审查加固（竞态/解析/防抖/资源十余处）、菜单新增「退出APP（保留服务）」（退壳不停服）、**壳原生会话完成通知（CompletionNotifier：WebSocket 直连服务事件流，托盘化期间照常通知）**、浏览器通知插件退役（桥接/权限放行/自愈死代码移除）
