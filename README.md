...
双击 `dsh-app.exe` → 自动拉起 `dsh web` 服务器 → 无地址栏独立窗口加载 Harness UI（v1.7.0 起携带 harness launch token 鉴权）→ 关闭窗口默认最小化到托盘(服务继续运行,双击图标恢复),菜单"退出"完全停服。

## 特性

- **自绘顶栏**(WindowChrome):纯文字工具按钮(刷新 / 浏览器 / 重启服务 / 菜单;日志入口在菜单内)
- **启动状态机**:5 步进度(探测 → 定位环境 → 拉起 → 就绪 → 加载),失败定位到具体步骤
- **断连检测**:自家 dsh 进程崩溃(`ServerDied`)即时感知,错误卡片 + 重试（v1.7.0 起接管模式归档,心跳随之退役）
...
- **会话完成通知**（v1.7.0 暂时禁用:harness v0.1.2-alpha.1 事件流迁移至多路复用协议,待重写适配）:模型输出完成(或出错)弹 Win11 系统通知,点击恢复窗口;壳直连服务事件流检测、不依赖页面,窗口托盘化/页面挂起期间照常生效;子代理后台任务不打扰;默认开,设置可关
- **局域网共享**（v1.7.0 暂时禁用:代理链路待适配 harness launch token,代码保留）:设置页一键开启后,手机/其他电脑浏览器访问 `http://电脑IP:3081/?key=密钥`(或扫设置页二维码)即可遥控同一 Harness(双端会话实时同步);token 门禁防蹭网,改设置/凭据等写操作仍仅限本机;托盘悬停显示 `LAN :端口`
- **系统托盘**:关窗最小化到托盘(服务继续,默认开,设置可关)、双击恢复、右键菜单(打开主窗口 / 检查 Harness 更新 / 检查应用更新 / 诊断信息 / 设置 / 退出)、悬停显示实时余额
- **菜单下拉**:关于 / 设置 / 日志 / 诊断信息 / 检查 Harness 更新 / 检查应用更新 / 退出(公共菜单控件,顶栏 / 托盘两处同源;胶囊分段 + 滑块开关,套用 Toolbox 设计)
...
| 服务器已在运行 | v1.7.0 起直接报错提示（harness launch token 仅存于拉起进程 stdout,外部进程无法取得,接管路径归档） |
| 3080 被占用 | 报错提示先关闭占用程序（随接管归档,不再自动换端口） |
| 服务运行中崩溃 | 错误卡片"服务连接已断开" + 重试(自家进程 Exited 事件感知) |
...
# 产物: bin\Release\net9.0-windows\win-x64\publish\dsh-app.exe (~174MB，含 ASP.NET Core 运行时供 LAN 共享)
```
