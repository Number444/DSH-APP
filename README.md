...
双击 `dsh-app.exe` → 自动拉起 `dsh web` 服务器(若未运行)→ 无地址栏独立窗口加载 Harness UI → 关闭窗口默认最小化到托盘(服务继续运行,双击图标恢复),托盘菜单"退出"才真正停服。
...
| 重复双击 / 多版本并存 | 单实例（互斥体 + 进程扫描双保险）,激活已有窗口后退出;不同构建版本（dev/publish）同样互斥 |
